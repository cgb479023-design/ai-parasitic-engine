...
try {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('// HYBRID APPROACH: Direct Value + Events (Restored & Enhanced)')) {
            startLine = i;
            break;
        }
    }
    ...
    if (startLine !== -1) {
        ...
        const newCode = `                                                try {
                                                    ...
                                                    const timeListItems = deepQueryAll(document.body, 'tp-yt-paper-item, .ytcp-text-dropdown-trigger');
                                                    const matchingItem = timeListItems.find(item => item.textContent.trim() === data.scheduleTime);
