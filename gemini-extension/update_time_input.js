...
try {
    ...
    // Find the block where we set the time input
    // We want to replace the direct .value setting with execCommand
    ...
    const newCode = `                                                try {
                                                    ...
                                                    console.log(\`✅ [Schedule] Time input updated via execCommand to: \${timeInput.value}\`);
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('// DIRECT VALUE SETTING (Verified to work!)')) {
            startLine = i;
            // We want to replace until the console.log line
            for (let j = i; j < lines.length; j++) {
                if (lines[j].includes('console.log(`✅ [Schedule] Time input successfully updated')) {
                    endLine = j;
                    break;
                }
            }
